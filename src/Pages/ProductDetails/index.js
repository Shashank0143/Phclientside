...
const ProductDetails = () => {
  ...
  const addtoCart = () => {
    ...
    if (activeSize === null && !isCustomTailoredSelected) {
      context.setAlertBox({
        open: true,
        error: true,
        msg: "Please select a size or custom tailored option before adding to cart.",
      });
      return; // Stop the function from proceeding if no size or custom tailored option is selected
    }
    // Prepare cart fields
    const cartFields = {
        ...
        size: isCustomTailoredSelected ? null : productData?.size[activeSize], // Send size or 'Custom Tailored'
        customMeasurements: isCustomTailoredSelected ? customMeasurements : null // Send custom measurements if selected
    };
              ...
              <div className="col-md-7 pl-5 pr-5 part2">
                ...
                <p className="mt-3"
                  style={{
                    ...
                    padding: "10px",
                  }}>{productData?.description?.slice(0, 230)}...</p>
                ...
                <Button style={{ background: "#4C7383", color: "#fff", marginTop:"10px" }} onClick={handleDialogOpen}>Custom Tailored</Button>
